...
//    private final org.jpedal.PdfDecoderFX _pdf = new org.jpedal.PdfDecoderFX();
    ...
    private void MyHandler(Object newVal)
    {
        ...
        {
            if (_CurrPage < _pdf.getPageCount())
            {
                _CurrPage++;
            }
            else if (_AutoLoopWithAdvance)
            {
                _CurrPage = 1;
            }
            else
        ...
        else
        {
            try
            {
                int newPage = Integer.parseInt(strVal);
                if (_pdf.getPageCount() < newPage)
                {
                    _CurrPage = newPage;
                    gotoPage(_CurrPage);
                }
            ...
            {
                LOGGER.warning("Received unknown item : [" + strVal + "] for PDF_ReaderWidget" + getName() + ": [" + getNamespace() + ":" + getMinionID() + "]");
                return;
            }
        }
        */
    ...
    private boolean setupPDF()
    {
        ...
        {
            LOGGER.warning("Both Height and Width specified for PDF Reader Widget.  Only one will be used; see docs.");
        }
        try
        {
            //Op    
            if (_SrcFile.startsWith("http"))
            {
                _pdf.openPdfFileFromURL(_SrcFile, false);
            }
            else
            {
